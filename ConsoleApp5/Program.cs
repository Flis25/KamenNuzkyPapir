...
bool start = false;
Console.WriteLine("Chceš si zahrát? (ano) (ne)");
string dzosef = Console.ReadLine();
dzosef.ToLower();
if (dzosef == "ano")
{
    AnsiConsole.Write(
    ...
    Console.WriteLine("Cool loading, right ? Ne? ... Byl tam jen pro swag :(((");
    Thread.Sleep(3300);
    Console.Clear();
...
{
    Console.WriteLine("Jsi si 100% jistý?");
    string tf = Console.ReadLine();
    tf.ToLower();
    if (tf == "ano")
    {
        Console.WriteLine("Ale, když si nechceš zahrát... Nemůžeme být spolu. To vážně chceš?");
        string tf2 = Console.ReadLine();
        tf.ToLower();
        if (tf2 == "ano")
        {
        ...
        {
            Console.WriteLine("Takže ses rozmyslel ? No... Já už ale nechci.");
            Thread.Sleep(2000);
        }
    }
    else
